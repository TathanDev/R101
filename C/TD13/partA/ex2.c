...
int partition(int tab[], int iMin, int iMax) {
    ...
    permute(&tab[i], &tab[iMax]);
    return i;
}


int partitionHoare(int tab[], int iMin, int iMax) {
    int pivot = tab[iMin];
    int i = iMin - 1;
    int j = iMax + 1;
...
void triRapide(int* tab, int iMin, int iMax) {
    if(iMin >= iMax || iMin < 0 || tab == NULL) {
        return;
    }

    int pivot = partitionHoare(tab, iMin, iMax);

    triRapide(tab, iMin, pivot -1);
    triRapide(tab, pivot + 1, iMax);
}
